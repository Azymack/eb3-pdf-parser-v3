...
def _build_messages(
    ...
    system_prompt = (
        ...
        f"For the Retail RX field: use the retail/30-day copay or cost for each tier. "
        f"For the Mail Order RX field: use the mail order/90-day cost — if the document shows "
        f"no separate mail order pricing (only one cost column) use null. "
        f"Omit tiers that explicitly say 'Not covered' for that network direction. "
        f"Example — 4-tier PPO: "
        ...
        f"In-Network Mail Order RX = null."
    )
