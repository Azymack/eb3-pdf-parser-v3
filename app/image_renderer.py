"""Render selected PDF pages to PNG images for the VLM.
...
def render_pages(pdf_bytes: bytes, page_numbers: list[int]) -> list[dict[str, Any]]:
    """Render the given 1-based page numbers from pdf_bytes to base64 PNGs.

    Returns:
        List of {"page_number": int, "image_b64": str, "width": int, "height": int}
    ...
    try:
        for page_num in page_numbers:
            ...
            logger.debug(f"image_renderer: rendered page {page_num} ({pix.width}x{pix.height}px)")
    finally:
        doc.close()
